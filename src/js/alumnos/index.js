...
const buscar = async () => {
  ...
  const url = `/final_IS2_cornelio/API/alumnos/buscar?alu_nombre=${alu_nombre}&${alu_apellido}&alu_grado=${alu_grado}&alu_arma=${alu_arma}&alu_nac=${alu_nac}`;
  const config = {
      method : 'GET'
  }
...
// const buscarNuevo = async (evento) => {
...
//           data: "id_alumnos",
//           render: (data, type, row, meta) => {
//             return `<a type="button" class="btn btn-warning" onclick="asignarValores('${row.id_alumnos}', '${row.alu_nombre}', '${row.alu_apellido}','${row.alu_grado}','${row.alu_arma}','${row.alu_nac}')">MODIFICAR</a>`;
//           },
//         },
//         {
//           data: "id_alumnos",
