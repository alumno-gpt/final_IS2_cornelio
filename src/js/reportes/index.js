...
const buscar = async () => {
  ...
  try {
    ...
    document.getElementById("nombre").innerHTML =
      data.alumno[0].alu_nombre + " " + data.alumno[0].alu_apellido;
    document.getElementById("grado").innerHTML =
      data.alumno[0].alu_grado + " / " + data.alumno[0].alu_arma;
    ...
    data.calificaciones.forEach((e) => {
      calificaciones.innerHTML += `<tr class="text-center"><td>${conteo}</td><td>${e.calif_materia}</td><td>${e.calif_punteo}</td></tr>`;
      conteo += 1;
    });
